...
two scariest bets early (the relay protocol pivot and the Swift client), and to keep
the server testable at every step even though the GUI clients are removed in Phase 0.
...
- Delete `apps/web`, `apps/desktop`, `apps/mobile`, `apps/marketing`, `infra/relay`,
  ...
  `packages/tailscale`, `packages/ssh`.
...
- Decide provider pruning (default: keep Claude + Codex first-class, keep the ACP
  and OpenCode drivers compiling since they're isolated; drop only what breaks).
...
## Phase 3 — Swift macOS client MVP

Start the client only after the protocol has stabilized against test clients.
Vault and app lock are foundations here, not retrofits.
...
- UI: channel sidebar with badges, channel stream + threads, agent interaction
  ...
  markdown/image viewing, owner admin screens (members, roles, channel ACLs).
...
- `MeshIntelligence`: SpeechAnalyzer voice-to-text, FoundationModels offline
  summarization, Translation framework — all operating on vault files with no
  network (whisper.cpp/MLX fallback path decided by a quality spike).
...
## Phase 5 — Artifact viewer and durable comments
...
- Optional airgapped agent driver (local models on the server, e.g. Ollama-backed)
  for zero-egress operation.
...
| No GUI between Phase 0 and Phase 3 ("dark period") | accepted by decision D10; CLI harness + scripted clients keep every phase demonstrable |
| Git can't hide paths within one repo | scoped out of v1 explicitly (§6); channel granularity is the read boundary, filtered mirrors in Phase 6 |
| secp256k1 keys can't live in the Secure Enclave | SE-wrapped master key + Keychain biometric access control (§9); documented rather than discovered late |
| Cloud agent providers are an inherent egress | per-channel Bot membership bounds exposure; airgapped driver in Phase 6 |
...
1. Final provider lineup (default assumption: Claude + Codex; ACP/OpenCode kept
   compiling until they cost something).
2. Exact 47xxx kind numbering and whether to upstream-align with Buzz's registry as
   it evolves.
3. Vault implementation (sparse bundle vs file-level crypto) — Phase 3 spike.
4. Forum-style channels (Buzz kinds 45001/45003) — not in v1.
5. macOS version floor (26 for the full local-AI set vs 15 + whisper.cpp fallback).
